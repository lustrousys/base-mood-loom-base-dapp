...
const source = await readFile(resolve(process.cwd(), "contracts/BaseMoodLoom.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseMoodLoom.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseMoodLoom.sol"]?.BaseMoodLoom;
...
console.log(`Deploying BaseMoodLoom from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_MOOD_LOOM_CONTRACT_ADDRESS to this contract address in Vercel.");
